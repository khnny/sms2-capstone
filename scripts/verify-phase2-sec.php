...
$wfSrc = (string) file_get_contents(ROOT_PATH . '/modules/crad/includes/research-coordinator-assignment-page.php');
$check(
    'WF-001',
    str_contains($wfSrc, 'CRAD_ALLOW_LEGACY_REGISTERED_ASSIGN')
        && str_contains($wfSrc, 'function cradAllowLegacyRegisteredAssignment'),
    'legacy assign gated by env flag'
);
...
$login = (string) file_get_contents(ROOT_PATH . '/login/login.php');
$check('SEC-007', str_contains($login, 'server-side only') && !str_contains($login, 'Trying: <code>'), 'login hides DB details');

$deploy = (string) file_get_contents(ROOT_PATH . '/setup/deploy-db.php');
$check('SEC-008', str_contains($deploy, 'SMS2_ALLOW_DB_IMPORT'), 'cloud DB import gated');
