...
function smsHandleAuthenticatorPost(int $userId, string $action, array $post, string $accountLabel = ''): array
{
    ...
    $row = null;
    if ($pdo) {
        $stmt = $pdo->prepare('SELECT id, email, full_name, password_hash FROM sms_users WHERE id = ? LIMIT 1');
        $stmt->execute([$userId]);
        $row = $stmt->fetch() ?: null;
    }
    if (!$row) {
...
function smsRenderAuthenticatorCard(int $userId, string $formActionUrl, string $csrfFieldHtml, bool $asBox = false): void
{
    ...
    $pdo = db();
    if ($pdo) {
        $st = $pdo->prepare('SELECT email, full_name FROM sms_users WHERE id = ? LIMIT 1');
        $st->execute([$userId]);
        $u = $st->fetch() ?: [];
